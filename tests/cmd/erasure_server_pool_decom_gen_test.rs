...
use minio_rust::cmd::{DecomError, PoolDecommissionInfo, PoolMeta, PoolSpaceInfo, PoolStatus};

pub const SOURCE_FILE: &str = "cmd/erasure-server-pool-decom_gen_test.go";
...
impl_cmd_codec!(PoolDecommissionInfo);
impl_cmd_codec!(PoolStatus);
impl_cmd_codec!(DecomError);
impl_cmd_codec!(PoolMeta);
impl_cmd_codec!(PoolSpaceInfo);
...
codec_tests!(pool_decommission_info, PoolDecommissionInfo);
codec_tests!(pool_status, PoolStatus);
codec_tests!(decom_error, DecomError);
codec_tests!(pool_meta, PoolMeta);
codec_tests!(pool_space_info, PoolSpaceInfo);
