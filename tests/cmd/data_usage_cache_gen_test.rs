...
use minio_rust::cmd::{
    AllTierStats, CurrentScannerCycle, DataUsageCache, DataUsageCacheInfo, DataUsageEntry,
    SizeHistogram, SizeHistogramV1, VersionsHistogram,
};

pub const SOURCE_FILE: &str = "cmd/data-usage-cache_gen_test.go";
...
impl_cmd_codec!(AllTierStats);
impl_cmd_codec!(CurrentScannerCycle);
impl_cmd_codec!(DataUsageCache);
impl_cmd_codec!(DataUsageCacheInfo);
impl_cmd_codec!(DataUsageEntry);
impl_cmd_codec!(SizeHistogram);
impl_cmd_codec!(SizeHistogramV1);
impl_cmd_codec!(VersionsHistogram);
...
codec_tests!(all_tier_stats, AllTierStats);
codec_tests!(current_scanner_cycle, CurrentScannerCycle);
codec_tests!(data_usage_cache, DataUsageCache);
codec_tests!(data_usage_cache_info, DataUsageCacheInfo);
codec_tests!(data_usage_entry, DataUsageEntry);
codec_tests!(size_histogram, SizeHistogram);
codec_tests!(size_histogram_v1, SizeHistogramV1);
codec_tests!(versions_histogram, VersionsHistogram);
