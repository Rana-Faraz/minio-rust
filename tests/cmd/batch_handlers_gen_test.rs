...
pub const SOURCE_FILE: &str = "cmd/batch-handlers_gen_test.go";

use minio_rust::cmd::{BatchJobInfo, BatchJobPrefix, BatchJobRequest};
...
impl_cmd_codec!(BatchJobInfo);
impl_cmd_codec!(BatchJobPrefix);
impl_cmd_codec!(BatchJobRequest);
...
codec_tests!(batch_job_prefix, BatchJobPrefix);
codec_tests!(batch_job_request, BatchJobRequest);
codec_tests!(batch_job_info, BatchJobInfo);
