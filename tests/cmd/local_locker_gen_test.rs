...
use minio_rust::cmd::{LocalLockMap, LockRequesterInfo, LockStats};

pub const SOURCE_FILE: &str = "cmd/local-locker_gen_test.go";
...
impl_cmd_codec!(LocalLockMap);
impl_cmd_codec!(LockRequesterInfo);
impl_cmd_codec!(LockStats);
...
codec_tests!(local_lock_map, LocalLockMap);
codec_tests!(lock_requester_info, LockRequesterInfo);
codec_tests!(lock_stats, LockStats);
