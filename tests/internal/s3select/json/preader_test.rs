...
use minio_rust::internal::s3select::json::{PReader, ReaderArgs};

pub const SOURCE_FILE: &str = "internal/s3select/json/preader_test.go";
...
#[test]
fn new_preader_matches_reference_cases() {
    for (name, input, expected_records) in sample_inputs() {
        let mut reader = PReader::new(Cursor::new(input.as_bytes()), &ReaderArgs)
            .unwrap_or_else(|err| panic!("failed to build preader for {name}: {err}"));
...
#[test]
fn new_preader_close_matches_reference_cases() {
...
#[test]
fn benchmark_preader_matches_reference_shape() {
    for (name, input, expected_records) in sample_inputs() {
        for _ in 0..10 {
            let mut reader = PReader::new(Cursor::new(input.as_bytes()), &ReaderArgs)
                .unwrap_or_else(|err| panic!("failed to build preader for {name}: {err}"));
